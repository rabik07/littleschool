...
class StudentController {
    static async list(req, res) {
        try {
            ...
            const students = className
                ? await Student_1.Student.getByClass(className)
                : await Student_1.Student.getAll();
            const classes = await Student_1.Student.getClasses();
    ...
    static async details(req, res) {
        try {
            const { id } = req.params;
            const student = await Student_1.Student.getById(id);
            if (!student) {
    ...
    static admissionForm(req, res) {
        try {
            // Provide classes for the form dropdown if needed
            Student_1.Student.getClasses().then((classes) => {
    ...
    static async submitAdmission(req, res) {
        try {
            ...
            const saved = await Admission_1.Admission.create({ data: admission, status: entry.status, submittedAt: entry.submittedAt });
    ...
    static async update(req, res) {
        try {
            ...
            if (rollNumber) {
                try {
                    const all = await Student_1.Student.getAll();
                    const conflict = all.find((s) => String(s._id) !== String(id) && s.rollNumber === rollNumber);
                    if (conflict) {
                        if (req.is('application/json'))
                            return res.status(400).json({ success: false, error: 'Roll number already in use by another student' });
            ...
            const updated = await Student_1.Student.update(id, {
                name,
                class: className,
                rollNumber,
                email,
    ...
    static async moveToHistory(req, res) {
        try {
            const { id } = req.params;
            const result = await Student_1.Student.delete(id);
            if (result) {
